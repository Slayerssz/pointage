...
--
--  Deux lettres de plus au pointage :
--     F    férié chômé        vaut 1 jour  (payé, personne ne travaille)
--     XF   férié travaillé    vaut 2 jours (la garde + celle du férié)
--
--  L'administrateur déclare un férié, du jour au jour, pour une société
--  ou pour tout le groupe. Le système inscrit aussitôt F à tout le monde.
--  Le validateur passe en XF ceux qui ont tenu le poste.
--
--  Ce qui n'est jamais écrasé : un jour déjà pointé (congé, maladie,
--  garde déjà saisie) et un mois clôturé. Le compte rendu vous dit
--  combien de jours sont dans ce cas.
--
--  Supprimer un férié retire exactement les F qu'il avait posés — ni les
--  XF, ni rien d'autre.
-- ============================================================================
...
-- select nom,
--        coalesce((select name from public.companies c where c.id = f.company_id),
--                 'TOUT LE GROUPE') as portee,
--        to_char(date_debut, 'DD/MM/YYYY') as du,
--        to_char(date_fin, 'DD/MM/YYYY')   as au,
--        (select count(*) from public.pointages p where p.ferie_id = f.id) as jours_inscrits
--   from public.jours_feries f
