...
--
--  ⚠ Si vous aviez déjà lancé une première version de ce BLOC 30 (celle
--    qui mettait les deux montants sur la fiche employé) : relancez
--    celui-ci par-dessus. Il défait proprement l'ancienne version, et
--    ne fait rien de plus si vous ne l'aviez jamais lancée.
--
--  Ces deux indemnités varient d'un mois à l'autre — untel a pris le bus
--  vingt jours, untel dix. Elles ne sont donc PAS sur la fiche employé :
--  elles se saisissent dans l'écran Paie, à côté de la Prime, de la Dette
--  et des Autres retenues, au moment où l'on arrête le mois.
--
--  Sur le bulletin, elles s'ajoutent APRÈS les retenues :
--     SALAIRE BRUT ................ 3 000,00
--     − C.N.S.S. / A.M.O. / I.G.R.
--     + FRAIS DE TRANSPORT ..........  300,00
--     + FRAIS DE PANIER .............  200,00
--     = GAIN NET
--
--  Elles n'entrent dans aucune assiette de cotisation. Le salaire brut et
--  les trois retenues sont donc inchangés : aucun bulletin déjà édité ne
--  bouge.
--
--  Régénérer un mois ne remet jamais ces montants à zéro : ils
--  appartiennent à la saisie de la paie, pas au pointage.
--
--  Le pied du bulletin indique enfin combien de jours fériés ont été
--  travaillés dans le mois (les XF du BLOC 29), chacun comptant double.
-- ============================================================================
...
--    Ce qui a été saisi en transport et panier, mois par mois.
-- ============================================================================

-- select c.name as societe, pp.annee, pp.mois,
--        count(*) filter (where lp.frais_transport > 0) as avec_transport,
--        count(*) filter (where lp.frais_panier > 0)    as avec_panier,
--        to_char(sum(lp.frais_transport), 'FM999999990.00') as total_transport,
--        to_char(sum(lp.frais_panier), 'FM999999990.00')    as total_panier
--   from public.lignes_paie lp
--   join public.periodes_paie pp on pp.id = lp.periode_id
--   join public.companies c on c.id = pp.company_id
--  group by c.name, pp.annee, pp.mois
